...
snakemake --snakefile ${binpath}/core/RIDGE_pipeline.py -p -j ${ntask_load} --until "model_filtering" --configfile ${1}  --config binpath=${binpath}  --latency-wait 60 --nolock
snakemake --snakefile ${binpath}/core/RIDGE_pipeline.py   -p -j ${ntask_load} --configfile ${1}   --config binpath=${binpath} --latency-wait 60 --nolock
else
