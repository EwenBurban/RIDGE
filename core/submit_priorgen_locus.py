...
full_locus_param_df = pd.concat(locus_param_df,axis=0)
full_locus_param_df.reset_index(inplace=True,drop=True)
full_locus_param_df.to_csv('priorfile_locus.txt',sep="\t",header=True,index_label='dataset',float_format='%.5f')
...
for sim in locus_param_df:
    ...
    output_command += '}' + ' | pypy {binpath}/mscalc_locus_2pop_SFS.py bpfile={bpfile} num_dataset={glob_index} \n'.format(binpath=binpath,bpfile=bpfile,glob_index=glob_index) 
