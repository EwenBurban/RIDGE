...
#### sim roc
scaled.sim_roc_ss=as.data.frame(apply(sim_roc_all[,col_data],2,function(x) normalise(x,x)))
scaled.obs_ss=as.data.frame(sapply(col_data,function(x,...) normalise(obs_data[,x],sim_roc_all[,x])))
scaled.sim_barrier=scaled.sim_roc_ss[sim_roc_all[,migration]==0,]
scaled.sim_nonbarrier=scaled.sim_roc_ss[sim_roc_all[,migration]!=0,]
rsim_vec_barrier=sapply(1:nrow(scaled.obs_ss),function(I,...){
				   dist_vec=euclidian_dist(target=scaled.obs_ss[I,,drop=F],sim=scaled.sim_barrier)
				   y=which(dist_vec==min(dist_vec))
				   return(y)})
rsim_vec_nonbarrier=sapply(1:nrow(scaled.obs_ss),function(I,...){
				   dist_vec=euclidian_dist(target=scaled.obs_ss[I,,drop=F],sim=scaled.sim_nonbarrier)
				   y=which(dist_vec==min(dist_vec))
				   return(y)})

sel_barrier=sample(1:nrow(obs_data),size=round(nrow(obs_data)*pbarrier),replace=F)
barrier_sim=rsim_vec_barrier[sel_barrier]
nonbarrier_sim=rsim_vec_nonbarrier[-sel_barrier]


test_set = as.data.frame(rbind(sim_roc_all[sim_roc_all[,migration]==0,col_data][barrier_sim,],sim_roc_all[sim_roc_all[,migration]!=0,col_data][nonbarrier_sim,]))
prior_test_set = as.data.frame(rbind(sim_roc_all[sim_roc_all[,migration]==0,col_prior][barrier_sim,],sim_roc_all[sim_roc_all[,migration]!=0,col_prior][nonbarrier_sim,]))

train_set = as.data.frame(rbind(sim_roc_all[sim_roc_all[,migration]==0,col_data][-barrier_sim,],sim_roc_all[sim_roc_all[,migration]!=0,col_data][-nonbarrier_sim,]))
prior_train_set = as.data.frame(rbind(sim_roc_all[sim_roc_all[,migration]==0,col_prior][-barrier_sim,],sim_roc_all[sim_roc_all[,migration]!=0,col_prior][-nonbarrier_sim,]))
...
sim_AUC=auc(sim_roc_stat$FPR,sim_roc_stat$TPR,absolutearea=F)
...
write.table(obs_prediction,file=file.path(obs_dir,'Pbarrier_rsim.txt'),sep='\t',row.names=F)
write.table(sim_roc_stat,file=file.path(obs_dir,'roc_table_rsim.txt'),sep='\t',row.names=F)
### report
...
if (mode=='test'){
	...
	write.table(roc_stat,file=file.path(obs_dir,'true_roc_table_rsim.txt'),sep='\t',row.names=F)

}
write.table(t(report),file.path(obs_dir,'report_barrier_detection_rsim.txt'),sep='\t',row.names=F)
