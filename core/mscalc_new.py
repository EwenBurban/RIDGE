...
if os.path.isfile('ABCjsfs.txt') == False:
    header = 'dataset\t'
    for i in range(nBins_spA):
...
for line in sys.stdin: # read the ms's output from the stdin
	...
	if nLoci_cnt != 0 and len(ss) == nLoci:
		...
		res = ''
		res += "{0}\t{1:.5f}\t{2:.5f}\t".format(num_dataset, bialsites_avg, bialsites_std)
...
outfile_jsfs = open("ABCjsfs.txt", "a")
outfile_jsfs.write(output_jsfs)
outfile_jsfs.close()

outfile_stat = open("ABCstat_global.txt", "a")
