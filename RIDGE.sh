...
## without slurm
snakemake --snakefile ${binpath}/core/RIDGE_V2_pipeline.py -p -j ${ntask_load} --configfile ${1}\
...
snakemake --snakefile ${binpath}/core/RIDGE_V2_pipeline.py -p -j ${ntask_load} \
	--config binpath=${binpath}  mode=${2} --configfile ${1} \
	--cluster-config ${binpath}/core/cluster.json \
	--cluster "sbatch --nodes={cluster.node} --ntasks={cluster.ntasks} --cpus-per-task={cluster.cpusPerTask} --time={cluster.time} --mem-per-cpu={cluster.memPerCpu} -p {cluster.p}" \
